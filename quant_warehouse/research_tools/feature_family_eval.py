    ...
    *,
    ...
    for symbol in symbols:
        ...
        try:
            ...
            # A single stale/malformed warehouse symbol must not abort a
            # broad live-scoring build. Keep the reason in diagnostics and
            # continue with the symbols that have usable history.
            frame, specs, diag = pd.DataFrame(), [], {
                "symbol": str(symbol).upper(),
                "status": "error",
                "error": f"{type(exc).__name__}: {exc}",
            }
            print(f"[feature-panel] skipped {str(symbol).upper()}: {diag['error']}", flush=True)
...
def _read_section(
    ...
    *,
    ...
    if section == "prices":
        frame = warehouse.read_prices(symbol, provider=provider)
    else:
        kwargs: dict[str, object] = {"section": section, "provider": provider}
        if period is not None:
            kwargs["period"] = period
        frame = warehouse.read_fundamentals(symbol, **kwargs)
    # Quant Warehouse's current API returns Polars frames; this legacy feature
    # recipe operates on pandas and must normalize at the boundary.
...
def _build_symbol_fundamental_panel(
    ...
    *,
    ...
    inputs["company_news"] = warehouse.read_news(
        symbol,
        provider=config.provider,
        start=pd.Timestamp(config.start_date).to_pydatetime() if config.start_date else None,
        end=pd.Timestamp(config.end_date).to_pydatetime() if config.end_date else None,
    )
    if hasattr(inputs["company_news"], "to_pandas"):
        inputs["company_news"] = inputs["company_news"].to_pandas()
