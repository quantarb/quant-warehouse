...
def backfill_calendar_and_etf_composition(
    *,
    ...
    etf_symbols = _etf_symbols(warehouse.config.catalog_path)
    composition_sections = [
        section
        for section in FMP_HISTORICAL_ETF_SECTIONS
        if section != "etf_nport_disclosure"
    ]
    if etf_symbols and composition_sections:
        if callable(progress_logger):
            progress_logger(
        ...
        counts = {"updated": 0, "skipped_fresh": 0, "error": 0, "other": 0, "total": len(results)}
        for row in results:
