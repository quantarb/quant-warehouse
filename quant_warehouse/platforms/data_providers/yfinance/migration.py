...
from quant_warehouse.platforms.data_providers.yfinance import (
    PROVIDER,
    ...
    *,
    ...
    for source_library in libraries:
        for source_symbol in _safe_list_symbols(source, source_library):
            ...
            target_library = target_library_for_legacy_symbol(catalog, source_library, symbol)
            target_symbol = symbol_provider_key(symbol, PROVIDER)
            try:
                if skip_catalog_funds and is_equity_fundamental_library(source_library) and should_skip_equity_fundamental_symbol(
                    catalog,
                    symbol,
                ):
                    rows.append(
