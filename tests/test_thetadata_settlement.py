...
def test_settle_option_exit_uses_last_contract_quote_before_expiration() -> None:
    quotes = {
        (pd.Timestamp("2021-02-04"), "GOOG_put_20210205_2015"): (0.10, 1.20, 0.65),
    }

    def quote_loader(symbol: str, date: pd.Timestamp, contract_symbol: str):
        assert symbol == "GOOG"
        return quotes.get((pd.Timestamp(date).normalize(), contract_symbol))
    ...
    settlement = settle_option_exit(
        ...
        equity_exit_date=pd.Timestamp("2021-02-16"),
    ...
    assert settlement.snapshot_date == pd.Timestamp("2021-02-04")
...
def test_settle_option_exit_allows_same_scale_intrinsic_fallback() -> None:
    settlement = settle_option_exit(
        ...
        option_type="call",
        strike=55.0,
        expiration=pd.Timestamp("2022-10-21"),
        equity_exit_date=pd.Timestamp("2022-11-01"),
        quote_loader=lambda symbol, date, contract_symbol: None,
    ...
    assert settlement is not None
    assert settlement.snapshot_date == pd.Timestamp("2022-10-21")
...
def test_iter_option_exit_lookup_dates_includes_weekend_target_then_prior_business_days() -> None:
    dates = iter_option_exit_lookup_dates(pd.Timestamp("2025-01-11"), 2)
