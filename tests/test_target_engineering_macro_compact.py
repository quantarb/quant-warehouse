...
def test_compact_macro_directional_labels_preserve_direction_and_fed_classes() -> None:
    tokens = pl.DataFrame({"date": ["2025-01-02"]})
    events = pl.DataFrame(
        {
            "date": ["2025-01-02"] * 4,
            "country": ["US"] * 4,
    ...
    assert panel[0, "is_macro_us_treasury_auction_increase"] == 1.0
    assert panel[0, "is_macro_us_consumer_spending_increase"] == 1.0
    assert panel[0, "is_fed_rate_cut"] == 1.0
...
def test_macro_event_release_time_joins_to_token_calendar_date() -> None:
    ...
    assert panel[0, "is_macro_us_cpi_increase"] == 1.0
