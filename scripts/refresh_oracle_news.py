...
def main() -> None:
    ...
    labels = pl.read_parquet(args.symbols_from_labels, columns=["symbol"])
    selected = select_oracle_trades(
        trades,
        symbols=labels["symbol"].unique().to_list(),
        k_values=(1, 2, 3),
        frequency="YE",
    ...
    print(
        f"trades={selected.height} boundaries={result.boundary_rows} "
